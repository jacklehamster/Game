...
function makeMat(base, array) {
    return array.map(function(num) {
        num = (10000+num).toString().slice(1);
        return 'animation/'+base+num+'.png';
    }).map(function(file) {
...
var sounds = new Array(20); var soundIndex = 0;

function playSound(snd) {
    if(!sounds[soundIndex]) {
        sounds[soundIndex] = new Audio();
    }
    sounds[soundIndex].src = 'snd/'+snd;
    sounds[soundIndex].play();
    soundIndex = (soundIndex+1) % sounds.length;
}

function stretch(scale, array) {
    var result = [];
    for(var i=0; i<array.length * scale; i++) {
        result[i] = array[Math.floor(i / scale)];
    }
    return result;
}
