(function (global, factory) {
    ...
    var coreGeometry, coreMesh, coreTexture;
    ...
    function initialize() {
        ...
        core.addLoop(1000/33, renderScene);
...
//var mesh = new THREE.Mesh( geometry, material );
            ...
            var count = 2;
            var vertexPerShape = 6;
			var cuts = new THREE.BufferAttribute(new Float32Array( 4 * count * vertexPerShape ), 4);
            geometry.addAttribute('cut', cuts);
//window.geo2 = geometry2;

//            uv.x = 0.0 + uv.x /myfloat + 1.0/myfloat;// + iGlobalTime;
  //          uv.y = 1.0 - 1.0/myfloat + uv.y /myfloat ;//+ 1.0/myfloat;
            var myfloat = 64.0;
            var cuts = geometry.attributes.cut.array;
            for(var i=0;i<count; i++) {
                for(var j=0;j<vertexPerShape;j++) {
                    cuts[i*vertexPerShape*4+j*4] = i==0?1/myfloat: 0.0;
                    cuts[i*vertexPerShape*4+j*4+1] = 1.0 - 1.0/myfloat;
                    cuts[i*vertexPerShape*4+j*4+2] = 1.0/myfloat;
                    cuts[i*vertexPerShape*4+j*4+3] = 1.0/myfloat;
                }
            }
    ...
    */
    ...
    core.makeProcess("apply-shaders",function(vertexShader, fragmentShader) {
        coreMesh.material = new THREE.ShaderMaterial( {
            uniforms: {
                texture:  { 
                    type: 'tv', 
