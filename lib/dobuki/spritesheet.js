(function (global, factory) {
     ...
     */
     ...
     function getTextureCount() {
        return coreTextures.length;
     }
     
    function getCut(frame, forCanvas) {
        core.expectParams(arguments, "object");
        var src = typeof frame.src == "string" ? frame.src : null;
        var cut = frame.cut;
        if(!cut || !Array.isArray(cut)) {
            cut = [0,0,0,0];
        }
    ...
    function initSpriteSheet(i) {
        ...
        var texture = coreTextures[i] = new THREE.Texture(canvas);        
...
//texture.minFilter = THREE.LinearFilter;        
   ...
   core.getTextureCount = getTextureCount;
