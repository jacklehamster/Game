(function (global, factory) {
    ...
    var array = [];
    var addedMouseHandler = false;
    function handleMouseDown(object, prop, parent, objectName) {
        var value = core.decodeObject(object[prop], object, prop);
        if(Array.isArray(value)) {
            value = value.join("\n");
        }
        delete object[prop];
        var func = new Function(value).bind(object);
        object.mouseDown = func;
        array.push(object);
        if(!addedMouseHandler) {
            addedMouseHandler = true;
    ...
    //  update mouse cursor
    core.watchModel("input.mouse.cursor.src",
        function() {
            ...
            if(newCursor && newCursor.src) {
                var customCursor = "url(" + newCursor.src + ") " 
                   + (newCursor.offset ? newCursor.offset.x + " " + newCursor.offset.y : "")
                   + "  , auto";
                ...
                cursorReplacement.style.marginLeft = (newCursor.offset ? -newCursor.offset.x : 0) + "px";
                cursorReplacement.style.marginTop = (newCursor.offset ? -newCursor.offset.y : 0) + "px";
            } else {
    ...
    core.registerModel("onMouseDown", handleMouseDown);
