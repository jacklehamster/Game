...
require(['threejs', 'dobuki'], function (THREE, DOK) {
    ...
    var images = {
        poop: [require.toUrl('poop.png'), require.toUrl('poop.png|scale:-1,1')],
        squid: {
        ...
        sand: [require.toUrl('walk.jpg'), require.toUrl('walk.jpg|scale:-1,1'), require.toUrl('walk.jpg|scale:1,-1'), require.toUrl('walk.jpg|scale:-1,-1')],
        dirt: [require.toUrl('walkshit.jpg'), require.toUrl('walkshit.jpg|scale:-1,1'), require.toUrl('walkshit.jpg|scale:1,-1'), require.toUrl('walkshit.jpg|scale:-1,-1')],
        /*        water: [
    ...
    */
    ...
    var spritePos = null,
        mouseMoveToVector = new THREE.Vector3();
    ...
    var zoombar = .4;
    var zoomState = [{ distance: 200, angle: 1.3 }, { distance: 1000, angle: .8 }];
    ...
    var hero = { x: 0, y: 0, z: 10, type: 'hero', speed: .5,
        last: 0
    };
    var coin = 0;
    ...
    var squid = { x: -3, y: -3, type: 'squid', touched: false, burst: 0,
        jump: { x: -3, y: -3 }
    };

    var actorsList = [hero, squid];
    ...
    function addSquid(hero) {
        ...
        if (!actorsList[1 + index % 300]) {
            var _squid = { x: x, y: y, z: z, dx: 0, dy: 0, dz: 0, type: 'squid', touched: false, burst: 0,
                jump: { x: x, y: y, z: z }, index: Math.floor(Math.random() * 10000)
            };
            actorsList[1 + index % 300] = _squid;
        } else {
            var _squid2 = actorsList[1 + index % 300];
            _squid2.x = x;
            _squid2.y = y;
            _squid2.z = z;
            _squid2.dx = 0;
    ...
    function addRunner(hero) {
        ...
        if (!actorsList[301 + index2 % 100]) {
            var _squid3 = { x: x, y: y, z: z, dx: 0, dy: 0, dz: 0, type: 'run', touched: false, burst: 0,
                jump: { x: x, y: y, z: z }, index: Math.floor(Math.random() * 10000)
            };
            actorsList[301 + index2 % 100] = _squid3;
            runners.push(_squid3);
        } else {
            if (!hero.touched) {
                return;
            }
            var _squid4 = actorsList[301 + index2 % 100];
            _squid4.x = x;
            _squid4.y = y;
            _squid4.z = z;
            _squid4.dx = 0;
    ...
    var actors = new DOK.Collection({
        ...
        if (actor.type === 'hero') {
            h = actor.z * 50;
            var img = mov.x < 0 ? DOK.SpriteSheet.spritesheet.hand.left : mov.x > 0 ? DOK.SpriteSheet.spritesheet.hand.right : hero.last ? hero.last : DOK.SpriteSheet.spritesheet.hand.right;
            hero.last = img;
            /*                        : mov.y < 0
        ...
        array[0] = DOK.SpriteObject.create(actor.x * cellSize, actor.y * cellSize, h, //c!==0?0:-64,
        cellSize * scale, cellSize * scale, null, img, light, //c!==0?1:1.5,
        wave //15,
        );
        array[0].type = 'face';
