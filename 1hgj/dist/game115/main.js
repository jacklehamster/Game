...
require(['threejs', 'dobuki'], function (THREE, DOK) {
    ...
    var range = 50;
    var cellSize = 256;
    engine.renderer.setClearColor(0 * 0xffffff, 1);
    ...
    */
    ...
    var hero = { x: 0, y: 0, type: 'hero', speed: .1 };
    var coin = 0;

    var startTime = 0;
    function showCoin() {
        var time = 40 - (DOK.Loop.time - startTime) / 1000;
        document.getElementById('sidebar').innerHTML = "$" + coin + "<br><font color='snow'>" + Math.floor(time * 100) / 100 + "</font>";
        if (time < 0) {
        ...
        for (var i = 0; i < actorsList.length; i++) {
            ...
            if (actor.type !== 'hero' && !actor.touched) {
                ...
                if (_dist < 1) {
                    ...
                    showCoin();
                } else if (_dist < 2 && !actor.touched && DOK.Loop.time - actor.burst > 50) {
    ...
    function addSquid() {
        ...
        var squid = { x: x, y: y, type: 'squid', touched: false, burst: 0,
            jump: { x: x, y: y }, index: Math.floor(Math.random() * 10000)
        };
        actorsList.push(squid);
    }
    ...
    var actors = new DOK.Collection({
        ...
        var wave = 0;
        if (actor.type === 'hero') {
            var img = mov.y === 0 && mov.x < 0 ? DOK.SpriteSheet.spritesheet.hand.left : mov.y === 0 && mov.x > 0 ? DOK.SpriteSheet.spritesheet.hand.right : mov.y < 0 ? DOK.SpriteSheet.spritesheet.hand.down : DOK.SpriteSheet.spritesheet.hand.up;
        } else if (!actor.touched) {
