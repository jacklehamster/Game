...
var bookmarklet = function bookmarklet() {
  ...
  var playGIF = function playGIF(gif) {
    var stream;
    var hdr;
    ...
    var doShowProgress = function doShowProgress(prefix, pos, length, draw) {
    ...
    var doLoadProgress = function doLoadProgress(e) {
    ...
    var doPlay = function () {
      ...
      var stepFrame = function stepFrame(delta) {
      ...
      var initToolbar = function initToolbar() {
         ...
         */ // Make compiler happy.
        var elt = function elt(tag, cls, attrs) {
        ...
        var updateTools = function updateTools() {
          if (playing) {
          ...
          var populate = function populate(elt, children) {
            elt.innerHTML = '';
          ...
          // XXX Blach.
          var simpleToolList = forward ? [showInfo, rev, prev, playPause, next, pin, close] : [showInfo, rev, next, playPause, prev, pin, close];
        ...
        var doRev = function doRev() {
          forward = !forward;
          updateTools();
          rev.focus(); // (because repack)
        };

        var doNextFrame = function doNextFrame() {
          stepFrame(1);
        };
        var doPrevFrame = function doPrevFrame() {
          stepFrame(-1);
        };

        var doPlayPause = function doPlayPause() {
          playing = !playing;
          updateTools();
        ...
        var doCurFrameChanged = function doCurFrameChanged() {
        ...
        var doCurDelayChanged = function doCurDelayChanged() {
        ...
        var doToggleShowingInfo = function doToggleShowingInfo() {
          showingInfo = !showingInfo;
          updateTools();
          showInfo.focus(); // (because repack)
        };

        var doTogglePinned = function doTogglePinned() {
          pinned = !pinned;
          updateTools();
  ...
  var insertCSS = function insertCSS(css) {
    // Surely there's a much better way of handling this whole thing.
  ...
  var to_a = function to_a(pseudoList) {
    // Blagh, delicious DOM. I'll do this for now.
    var a = [];
  ...
  var join = function join(strings, between) {
    if (between === undefined) between = '';
    return strings.reduce(function (s, n) {
      return s + between + n;
    }, '');
  };
  ...
  var removeClass = function removeClass(elem, cls) {
    var classes = elemClasses(elem).filter(function (_cls) {
      return _cls !== cls;
    });
  ...
  var mkOverlay = function mkOverlay(gif) {
  ...
  var rmOverlay = function rmOverlay(gif) {
